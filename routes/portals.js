...
router.get('/', async (req, res) => {
  try {
    const portals = await portalQueries.findByUser.all(req.session.userId);
    ...
    console.error('List portals error:', err);
...
router.post('/', async (req, res) => {
  try {
    ...
    const result = await portalQueries.create.run(
      req.session.userId,
      name.trim(),
      login_url.trim(),
      marksheet_url.trim(),
      (logout_url || '').trim(),
      (username_selector || '#MainContent_UserName').trim(),
      (password_selector || '#MainContent_Password').trim(),
...
router.put('/:id', async (req, res) => {
  try {
    ...
    const result = await portalQueries.update.run(
      name.trim(),
      login_url.trim(),
      marksheet_url.trim(),
      (logout_url || '').trim(),
...
router.delete('/:id', async (req, res) => {
  try {
    const result = await portalQueries.delete.run(parseInt(req.params.id), req.session.userId);
    if (result.rowsAffected === 0) {
      return res.status(404).json({ error: 'Portal not found.' });
    }
    res.json({ success: true });
